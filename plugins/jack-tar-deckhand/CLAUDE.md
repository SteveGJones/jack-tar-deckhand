...
| `/iterate-slide` | Single-slide critique-driven refinement via paperbanana `--continue-run` (three modes: auto / enumerate / draft), PLUS a fourth **edit channel** (#143): a $0 local mflux targeted edit that preserves everything the operator's feedback doesn't name — offered whenever an mlx edit backend + base image are available and the feedback is spatially local (never for text-correction feedback — hard-excluded, routes to re-roll or `/annotate-figure` native mode instead) |
| `/annotate-figure` | Perfect-text labeled figures — standalone raster overlay, or deck-native editable text boxes + connectors via `annotation_mode: native` (v2.1: `composed` strategy keeps chrome and draws into the image zone; `annotation.show_headline` opts a headline band into full-slide native slides, #142 v2.1). Blank-zone variant (#142 final scope item): `annotation.blank_zone` (`left_third\|right_third\|top_strip\|bottom_strip\|auto`) reserves a deliberately empty region of the base image — the render prompt asks for it, the anchor pass verifies it, `place_labels_in_zone` stacks labels inside it when verified clear with capacity, and everything falls back to the standard margin bands otherwise (never blocks, never re-renders, $0). Usually honoured by local models (P1 dogfood outcome: 75–96% compliance, +54 to +67 pt lift over controls — `docs/superpowers/dogfooding/2026-07-24-blank-zone-compliance.md`); the verification + fallback carry the guarantee, not the prompt. Native slides: bridge Step 4.8 owns it; raster/standalone: the `/annotate-figure` flow owns it (BZ-1). |
...
**Edit channel (issue #143).** A fifth, $0 refinement action — a local mflux targeted edit — sits alongside `revise_prose` / `refine_prompt` / `escalate_tier` once at least one attempt has rendered (`iterate_slide_hooks.can_edit`). It is proposed automatically by `decide_next_action` when the Director's Critic returns `refine_at_tier` on a spatially local gap, but F12 still fires an unconditional operator gate on every edit iteration — an edit being free does not exempt it from image-level review. Text-correction feedback is hard-excluded from this channel (never offered, not even with a warning) — route those to `refine_prompt`/`escalate_tier` or, for annotated slides, `/annotate-figure` native mode. See `src/edit_dispatch.py` and `docs/superpowers/plans/2026-07-23-edit-tier.md`.
