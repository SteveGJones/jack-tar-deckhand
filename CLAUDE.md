...
### Current Status (2026-07-24 — edit tier PR D shipped on branch, issue #143)

- **`feat/edit-tier-loop` wires the $0 local mflux edit action (PR C, `edit_image.py` + catalog fields + `/jack-tar-mlx:image-edit`, already on main) into the two critique loops.** New `plugins/jack-tar-deckhand/src/edit_dispatch.py` — `detect_mlx_edit_backend` (role/entrypoint/edit-RAM-gate filtered probe, sibling to `paperbanana_dispatch.detect_mlx_backend`), `edit_channel_available` (D8 FIRM — mflux/ollama/cloud base provenance ALL allowed, recorded not gated), `edit_channel_unavailable_reason` (F-06 — distinguishes a stale `~/.jack-tar/model-catalog.json` cache missing the `image_edit` role from a plain "no backend"), `classify_edit_locality` (D9 FIRM — text-targeting feedback is a HARD EXCLUSION, applied before any local/global cue match; never offered-with-warning), `build_edit_args` (F-08 — seed always resolved; no width/height keys per the S7 mflux-hang ruling), `record_edit` (D5 — `edit_chain` provenance, parent hash + instruction + args + parent provenance class, per edit).
- **iterate-slide** (`plugins/jack-tar-deckhand/src/iterate_slide_dispatch.py`): new "Edit channel" section + `edit_action` helper (flat image-manifest analogue of `update_manifest_entry`, chains `edit_chain` instead of `paperbanana_history`); `available_channels_for_creative_vision` appends `"edit"` when the manifest's `can_edit` hook is True; the existing F4 annotation-refresh guard (`annotation_refresh_required`/`annotation_refresh_notice`) fires on an edit-triggered replacement with ZERO new code — it only ever reads the strategy-map's `annotation_mode`.
- **creative_vision cascade**: `manifest.py` — `iterate_slide_hooks.can_edit` (False → True after the first attempt with an on-disk render); `append_attempt`'s `if tier != "mlx_edit"` guard leaves `current_tier`/`next_tier_available` untouched so a later `escalate_tier` resumes from wherever the ladder actually was. `cascade.py` — `"mlx_edit": (None, None, None)` in `TIER_TO_PROVIDER_MODEL_RESOLUTION` (F-10 — derives `TIER_COSTS["mlx_edit"] == 0.0`; never returned by `next_tier`; the cost-reconciliation integration test is unaffected since it filters to google/recraft providers). `orchestrator.py` — `decide_next_action` gains optional `locality`/`can_edit` kwargs; returns `NextAction(kind="edit")` only when `verdict == "refine_at_tier"` AND `locality == "local"` AND `can_edit` — defaults preserve pre-#143 behaviour exactly. `should_fire_operator_gate` needed NO code change: `strategy == "creative_vision"` already fires unconditionally (F12 absolute), the mlx_edit `TIER_COSTS` entry just keeps that lookup from `KeyError`-ing.
- **Schema**: `creative_vision_manifest.schema.json` gains optional `attempt.base_attempt_index`/`base_image_hash` and `iterate_slide_hooks.can_edit` properties (documentation-quality additions — the schema has no `additionalProperties: false` anywhere, so nothing was strictly REQUIRED for validation to pass; added anyway per the design's explicit ask). `tier` was already an unconstrained string, so `"mlx_edit"` needed no enum change.
- **imagegen-bridge** gains **Step 4.9: Local edit** — the shared mechanics (availability check surfacing the F-06 reason, text-carve-out classification, wrapper invocation via the sibling `jack-tar-mlx` plugin reusing the `MLX_PLUGIN_ROOT`/`MFLUX_REPO_USED` pattern from Step 4.6, image-reviewer dispatch, F12-unconditional gate for creative_vision, persistence via `edit_action` or `append_attempt`) reachable from both the academic_figure local-draft loop and the creative_vision loop.
- **Genuine design-doc reconciliation (smallest-reasonable-call, not a relitigation)**: the design's F-09 illustrative attempt shape uses `render.{prompt, image_path}` while the pre-existing (pre-#143) `render` contract requires `{model, resolution, cost_usd, output_path}` — a mismatch neither this PR nor PR C's smoke plan surfaced, because no prior test ever schema-validated a POPULATED `attempts` array (all prior schema tests used `attempts: []`). Resolution: edit attempts populate BOTH key sets (schema's `additionalProperties` is open, so this is additive, not a schema change) — `test_manifest_accepts_mlx_edit_attempt_shape` pins the reconciled shape. `manifest.append_attempt`'s `can_edit`-flip check reads `render.image_path` OR `render.output_path` for the same reason.
- **Tests**: deckhand suite 715 → **741 passed** (1 skipped, unchanged) — 44 new in `test_edit_dispatch.py` + net new/extended assertions across `test_creative_vision_{cascade,manifest,orchestrator,schemas,ga_flow,iterate_slide}.py` and `test_iterate_slide_dispatch.py`. Cross-plugin integration suite 101 → **106 passed** (`test_imagegen_bridge_skill.py` Step 4.9 drift pins).
- **Design doc**: [`docs/superpowers/plans/2026-07-23-edit-tier.md`](docs/superpowers/plans/2026-07-23-edit-tier.md) (adversarially reviewed, GO-WITH-CHANGES, 4 MAJOR + 8 MINOR all dispositioned §13; smoke plan executed 2026-07-23, all decisions FIRM §8.1). **PR C** (already on `main`): `edit_image.py` wrapper + catalog `image_edit` role/fields + `/jack-tar-mlx:image-edit` skill, `jack-tar-mlx` 0.1.0 → 0.2.0. Deckhand bumped `1.12.0 → 1.13.0` (marketplace lockstep) — as the blank-zone entry above predicted.
- **Deferred (design §7/§10, unchanged)**: cloud edit endpoints (Gemini/FAL) — no cost advantage over a regeneration, seam reserved (`backend: "mlx_edit"` generalises to `cloud_edit"` without schema churn). qwen-edit live validation on <64 GB machines. Editing non-raster/SmartArt/chart slides.

### Current Status (2026-07-24 — blank-zone annotation variant shipped on branch, issue #142 FINAL scope item)

- **`feat/annotate-blank-zone` implements the last #142 scope item**: `annotation.blank_zone` (`left_third | right_third | top_strip | bottom_strip | auto`) reserves a deliberately EMPTY region of the base image for label placement. Three cooperating parts: a prompt-side composition directive (positive space-claim + short emptiness clause, before the no-text negative), a new deterministic `place_labels_in_zone` placement engine (all labels stacked in the zone, height + width capacity gates BZ-2/BZ-3/BZ-6, all-or-nothing `None` → fallback), and one zone-verification question folded into the amended THREE-KEY anchor-pass contract (BZ-10 — the single "Output ONLY JSON" line enumerates `description`+`anchors`+`blank_zone`; `parse_blank_zone_verdict` is tolerant, absent/malformed = not clear = fallback). Best-effort by design: busy zone / unverified / over-capacity all fall back to the standard v2 margin-band flow — never blocks, never auto-re-renders, no new operator gate ($0 local throughout). Deckhand bumped `1.11.0 → 1.12.0` (marketplace lockstep). **Note for the edit-tier plan (PR D)**: this release takes 1.12.0 — PR D's "next available minor" shifts to **1.13.0**.
  - **Mode ownership (BZ-1)**: `native` slides — imagegen-bridge Step 4.8 (directive at Step 1, amended contract at Step 2, `blank_zone`/`blank_zone_clear` pass-through at Step 5); `raster` + standalone — entirely inside the `/annotate-figure` flow (§1 directive, §2 amended contract, §3 zone-preferred explicit placements into `annotate()`). The bridge never computes raster placements.
  - **Audit**: payload gains an optional `blank_zone` block (`requested`/`resolved`/`verified_clear`/`placement`, all four explicit per F9, absent entirely when not requested — v2/v2.1 payloads byte-identical). QA unchanged (placement-agnostic by design); F4 refresh-instructions constant gains the blank-zone re-run extension (BZ-4) so `/iterate-slide` rebuilds cannot silently revert zone placement to margins.
  - **$0 dogfood (P1 outcome)**: 24 directive + 6 control local renders (Ollama klein-9b + mflux z-image-turbo × 4 zones × 3 scenes). Compliance 96% (v1 question wording) / 75% (calibrated v2 wording), per-zone lift vs matching controls +54 to +67 points (BZ-8), no zone ≤ 1/6. Reviewer question RECALIBRATED mid-dogfood: first-pass wording's "water counts as clear" was read literally over breaking surf (uniformly lenient false-clears); shipped v2 wording (calm-water + surf-is-busy + whole-region rule) moves residual error to the conservative fallback side. Honoured + fallback end-to-end paths both driven through the real code path and preview-reviewed. Log: [`docs/superpowers/dogfooding/2026-07-24-blank-zone-compliance.md`](docs/superpowers/dogfooding/2026-07-24-blank-zone-compliance.md). Calibration: fractions 0.33/0.25 KEPT, directive texts KEPT, `auto`→`right_third` KEPT, pixel metric NOT promoted (OQ-3).
  - **Design doc**: [`docs/superpowers/plans/2026-07-23-annotate-blank-zone.md`](docs/superpowers/plans/2026-07-23-annotate-blank-zone.md) (adversarially reviewed, GO-WITH-CHANGES, BZ-1..BZ-10 dispositioned in §14).
