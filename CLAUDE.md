...
The model catalog entry notes carry these findings (gemini-3-pro-image, gemini-3.1-flash-image, mlx/z-image-turbo). ERNIE catalog entry + routing updates tracked as follow-up issues. Operators can query this doctrine interactively via `/jack-tar-deckhand:model-advisor` (deckhand 1.9.0).
